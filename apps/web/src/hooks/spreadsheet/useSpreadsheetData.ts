  ...
  for (let rowOffset = 0; rowOffset < values.length; rowOffset++) {
    for (let colOffset = 0; colOffset < values[rowOffset].length; colOffset++) {
      ...
      const belongsToOrigin =
        target?.spillParent?.row === originRow &&
        target.spillParent.col === originCol;
...
function rewriteFormulas(data: SheetData, change: StructuralChange): void {
  Object.keys(data).forEach((rowKey) => {
    ...
    Object.keys(row).forEach((colKey) => {
      ...
      if (cell?.formula)
        cell.formula = rewriteFormulaForStructuralChange(cell.formula, change);
    });
  });
}
...
export function useSpreadsheetData({
  ...
  const workbookFor = useCallback(
    (current: SheetData): FormulaWorkbook | undefined =>
      currentSheetName
        ? { ...workbook, [currentSheetName]: current }
        : workbook,
  ...
  const applyChange = useCallback(
    (recipe: (draft: SheetData) => void) => {
      ...
      if (patches.length > 0) {
        ...
        onLocalCellsChange?.(
          collectPersistedCellUpdates(data, nextState, patches),
        );
      }
  ...
  const updateData = useCallback(
    (newData: SheetData) => {
      ...
      if (!worker) return;
      void worker
        .calculate(newData, namedRanges, workbookFor(newData))
        .then((calculated) => {
  ...
  const applyExternalData = useCallback(
    (updater: (current: SheetData) => SheetData) => {
      workerGenerationRef.current++;
      setData((current) => {
        const next = updater(current);
        queueMicrotask(() => onDataChange?.(next));
        return next;
      });
    },
    [onDataChange],
        ...
        const next = produce(current, (draft) => {
          if (change.axis === "row") {
            const rows = Object.keys(draft)
              .map(Number)
              .sort((a, b) => (change.type === "insert" ? b - a : a - b));
            if (change.type === "insert") {
              for (const row of rows)
                if (row >= change.index) {
                  draft[row + 1] = draft[row];
                  delete draft[row];
                }
              draft[change.index] = {};
            } else {
              delete draft[change.index];
              for (const row of rows)
                if (row > change.index) {
                  draft[row - 1] = draft[row];
                  delete draft[row];
          ...
          rewriteFormulas(draft, change);
          const next = draft as unknown as SheetData;
          recalculate(next, namedRanges, undefined, workbookFor(next));
        });
        queueMicrotask(() => onDataChange?.(next));
        return next;
      });
    ...
    (updates: { row: number; col: number; value: string }[]) => {
      applyChange((draft) => {
        updates.forEach(({ row, col, value }) => {
          if (!canEditCell(protectedRanges, { row, col }, currentUserId))
            return;
          if (!draft[row]) draft[row] = {};
  ...
  const updateRichCells = useCallback(
    (updates: { row: number; col: number; cell: CellData }[]) => {
      applyChange((draft) => {
        ...
        updates.forEach(({ row, col, cell }) => {
          ...
          draft[row][col] = {
            ...
            validation: cell.validation
              ? structuredClone(cell.validation)
              : undefined,
  ...
  const handleUndo = useCallback(() => {
    if (historyIndex >= 0) {
      ...
      onLocalCellsChange?.(
        collectPersistedCellUpdates(data, nextState, inversePatches),
      );
      onDataChange?.(nextState);
    }
  ...
  return {
    ...
    removeProtectedRange: (id: string) => {
      setProtectedRanges((current) =>
        current.filter(
          (item) => item.id !== id || item.ownerId !== currentUserId,
        ),
      );
    },
    namedRanges,
    defineNamedRange: (name: string, range: CellRange) => {
      ...
      if (
        !/^[A-Z_][A-Z0-9_.]*$/.test(normalizedName) ||
        /^\$?[A-Z]+\$?[1-9][0-9]*$/.test(normalizedName)
      ) {
        throw new Error(
          "이름은 문자 또는 밑줄로 시작해야 하며 셀 주소와 같을 수 없습니다.",
        );
      }
    ...
    history,
    sortRows: useCallback(
      (colIndex: number, ascending: boolean = true) => {
        applyChange((draft) => {
          const currentData = draft as unknown as SheetData;
          sortRowsData(
            currentData,
    ...
    // Find and Replace
    updateCellFormat,
    findNext: useCallback(
      (
        query: string,
        matchCase: boolean,
        startIndex: { row: number; col: number },
      ) => {
        ...
        for (const r of rows) {
          ...
          for (const c of cols) {
            ...
            if (
              r < startIndex.row ||
              (r === startIndex.row && c <= startIndex.col)
            )
              continue;
        ...
        applyChange((draft) => {
          for (let row = range.start.row; row <= range.end.row; row++) {
            for (let col = range.start.col; col <= range.end.col; col++) {
              ...
              const dataRowIndex = config.hasHeader
